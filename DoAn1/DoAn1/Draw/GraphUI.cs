   ...
   public class GraphUI : Control
    {
        ...
        public Node SelectedNode
        {
            get
            {
                if (_selectedIndex < 1)
                    return null;
                return Controls[_selectedIndex] as Node;
            }
        }

        internal DrawingTools Tool
        {
            get => this.Tool;
            set => this.Tool = value;
        }
        ...
        void GetPath(int x, int y)
        {
            ...
            if (i == -1)
            {
                ...
                Point pos = Controls[y + 1].Location;
            ...
            else
            {
                ...
                _preNodeIndex = i;
                _ball.NodeLocations.Add(Controls[i + 1].Location);
                GetPath(i, y);
            }
        }
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                else if (tool == DrawingTools.Edge || tool == DrawingTools.Eraser)
                {
                    int count = 0;
                    foreach (var edge in _edges)
                    {
                        var start = Controls[edge.Start].Location;
                        start.X += NODE_RADIUS;
                        start.Y += NODE_RADIUS;
                        var end = Controls[edge.End].Location;
                        end.X += NODE_RADIUS;
                        end.Y += NODE_RADIUS;
        ...
        public void DeleteSelectedNode()
        {
            ...
            Node n = Controls[_selectedIndex] as Node;
            n.DoRemovingAnimation();
            Controls.RemoveAt(_selectedIndex);
